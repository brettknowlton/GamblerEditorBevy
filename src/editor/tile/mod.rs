...
fn tilemode_keybinds(
    ...
    if input.just_pressed(KeyCode::KeyP) {
        ...
        Tile::place(&mut commands, focused_item, coord);
...
fn load_spritesheet(mut commands: Commands, asset_server: Res<AssetServer>, mut message_queue: ResMut<EditorBottomBarQueuedMessages>) {
    //load the tilesheet for this mode
    let tex_path = PathBuf::from("textures/tiles/tilesheet.png");
    send_message!(Some('i'), message_queue, format!("Tilesheet Loaded: \"{}\"", tex_path.display()));
