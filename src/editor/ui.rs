...
pub fn egui_panel_render(
    ...
    if !tooling_menu.visible || !matches!(editor_state.get(), EditorState::Editing(_)) {
        return Ok(());
    }
    ...
    let panel_width = if is_tile_mode {
        ...
        (columns as f32 * tile_button_px) + ((columns.saturating_sub(1)) as f32 * tile_spacing) + 24.0
    } else {
        220.0
    };
    ...
    egui::SidePanel::left("tooling_menu_panel")
        ...
        .show(ctx, |ui| {
            ui.heading(tooling_menu.title.clone());
            ui.separator();
                    ...
                    egui::Grid::new("tooling_grid")
                        ...
                        .show(ui, |ui| {
                            for (i, item) in items.iter().enumerate() {
                                ...
                                if let (Some(texture_id), Some(tile_rect)) = (tile_texture_id, item.rect) {
                                    let atlas_w = (SPRITESHEET_WIDTH * TILE_SIZE as u64) as f32;
                                    let rows = ((MAX_SPRITESHEET_ITEMS + SPRITESHEET_WIDTH - 1)
                                        / SPRITESHEET_WIDTH) as f32;
    ...
    if tooling_menu.selected_item_id != next_selected_id {
        tooling_menu.selected_item_id = next_selected_id;
...
pub fn send_messages(
    ...
    if let Some((_, message)) = queued_messages.messages.first() {
        if let Ok((_, mut t)) = display_message.single_mut() {
            **t = format!("{message}",);
        }
    }
}
...
pub fn spawn_general_editor_ui(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    let x_size = DEFAULT_WINDOW_WIDTH as f32;
    let y_size = 30.0;
    ...
    // create tooling menu root node, this will be the parent of all our tooling menu items, and we can hide/show the entire menu by hiding/showing this root
    commands
        .spawn((
            ToolingMenuRoot,
            Visibility::Hidden,
            Node {
                ...
                top: Val::Px(8.0),
                left: Val::Px(8.0),
                width: Val::Px(DEFAULT_WINDOW_WIDTH as f32 / 5.0),
                max_height: Val::Px(DEFAULT_WINDOW_HEIGHT as f32 - 48.0),
            ...
            BorderColor::all(Color::srgba(0.85, 0.9, 0.95, 0.6)),
        ))
